...
| Embedding | OpenAI text-embedding-v4 | — | 当前仓库实际接入模型，接口稳定，便于与现有实现保持一致 |
...
sse-starlette>=2.0
pymupdf>=1.24       # PDF 解析
docx2txt>=0.8       # DOCX 解析依赖
python-docx>=1.1    # 其他 DOCX 处理场景可复用
```
...
from app.modules.knowledge_base.rag_chain import FALLBACK_ANSWER, rag_query
...
> 当前 `tests/test_rag_chain.py` 主要验证 `rag_query()` / `rag_query_stream()` 的分支逻辑：无检索结果时 fallback、命中后 answer + sources、source 截断，以及流式 chunk 透传；它通过 mock `search()` / chain 隔离真实向量库和 LLM。
>
...
> 当前知识库相关迁移链不只包含“补字段”，还包括 `(collection_name, file_hash)` 唯一约束、`updated_at` 和 `similarity_fingerprint`；核对 `knowledge_documents` 表结构时应以当前模型终态为准，而不是只看早期字段列表。
...
> 1. 文档上传后，用 PyMuPDF / Docx2txt / Text / Markdown loader 解析文本
> 2. 用 RecursiveCharacterTextSplitter 按 500 字分块，重叠 100 字保留上下文
> 3. 用当前配置的 OpenAI 兼容 embedding 模型把每个块向量化，存入 ChromaDB
...
- **上传幂等 + 近重复确认**：`(collection_name, file_hash)` 唯一约束负责精确判重，文件级 SimHash + `confirm_upload` 负责近重复确认；并发冲突仍走 409
