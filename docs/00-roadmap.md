...
| 第 2 周后半 | 检索增强问答 + SSE 流式响应 | RAG 知识库可用（当前先提供弱追溯来源展示，后续再补 upload record 驱动的强追溯） |
...
| 周 | 任务 | 产出 |
|---|---|---|
| 第 3 周 | 模拟面试：session 管理 + Skill 蓝图出题 + 难度 rubric + 自适应追问 planner + 主追问聚合评估 | 模拟面试可用 |
| 第 3 周末 | 面试安排：规则 + AI 双引擎邀请解析 | 面试邀请解析可用 |
