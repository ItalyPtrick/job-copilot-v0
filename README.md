...
> **当前进度**：W1 数据层 + W2 知识库全部完成（含 `/kb/*` 4 个接口、上传幂等、Orchestrator RAG 注入、Alembic 迁移、测试套件 25 passed）。W3 模拟面试模块计划已生成（`Today_Plan/W3/D1.md ~ D7.md`），等待实施。
...
- 知识库上传具备内容哈希幂等保护（同文件不重复 embedding，并发冲突返回 409）
...
- `/kb/upload`：以“响应 + `knowledge_documents` 记录 + `data/uploads/` 落盘文件”三点交叉验证成功，不只看 Swagger UI 单一展示；同文件二次上传应返回 `reused: true` 且不重复 embedding；并发冲突返回 409
