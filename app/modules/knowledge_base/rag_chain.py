...
def _build_sources(documents):
    # sources 来自检索层，不依赖模型生成，作为后续接口层的稳定展示依据
    return [
        {
...
# 非流式查询：检索 -> 拼 context -> 一次性生成 -> 回 sources
